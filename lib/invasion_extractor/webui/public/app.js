let currentView = 'all';
    ...
    function updateTitle() {
      ...
      if (currentView === 'group-detail') {
        ...
        const filterLabel = { everything: 'Everything', unassigned: 'Unassigned', assigned: 'Assigned', deleted: 'Deleted' };
    ...
    async function reorderClip(oldIndex, newIndex) {
      const res = await fetch('/api/reorder', {
        ...
        body: JSON.stringify({ group: selectedGroup, old_index: oldIndex, new_index: newIndex })
      });
      if (res.ok) {
        await loadClips();
      } else {
    ...
    async function openClipFile() {
      if (!selectedClipId) return;
      const res = await fetch('/api/clip/' + encodeURIComponent(selectedClipId) + '/open', { method: 'POST' });
    ...
    async function saveNote() {
      ...
      const res = await fetch('/api/note', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: selectedClipId, note: note })
      });
      if (res.ok) {
    ...
    async function saveTitle() {
      ...
      const res = await fetch('/api/title', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: selectedClipId, title: title })
      });
      if (res.ok) {
    ...
    function markCutEnd() {
      ...
      currentCuts.push({ start: currentCutStart, end: end });
    ...
    async function saveCuts() {
      ...
      const res = await fetch('/api/cuts', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: selectedClipId, cuts: currentCuts })
      });
      if (res.ok) {
    ...
    async function deleteSelected() {
      ...
      const res = await fetch('/api/clip/' + encodeURIComponent(selectedClipId), { method: 'DELETE' });
      if (res.ok) {
        selectedClipId = null;
        resetEditor();
        await loadClips();
    ...
    async function setResult(result) {
      ...
      const res = await fetch('/api/result', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: selectedClipId, result: result })
      });
      if (res.ok) {
        renderResult(result);
    ...
    async function setRating(rating) {
      ...
      const res = await fetch('/api/rating', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: selectedClipId, rating: rating })
      });
      if (res.ok) {
