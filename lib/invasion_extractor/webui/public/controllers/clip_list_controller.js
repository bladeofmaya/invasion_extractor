...
export default class extends ApplicationController {
  ...
  selectedClipIdValueChanged() {
    this.containerTarget.querySelectorAll('.clip-item').forEach(el => {
      el.classList.toggle('selected', el.dataset.id === this.selectedClipIdValue)
    })
  }
  ...
  render() {
    ...
    if (this.clips.length === 0) {
      ...
      container.innerHTML = '<div class="empty-state">No clips here. ' + message + '</div>'
      return
    }

    this.clips.forEach((clip, index) => {
      const div = document.createElement('div')
      div.className = 'clip-item' + (clip.id === this.selectedClipIdValue ? ' selected' : '')
      ...
      const resultBadge = clip.result ? '<span class="result-badge ' + clip.result + '">' + clip.result + '</span>' : ''
      const displayTitle = clip.title ? clip.title : clip.filename

      let groupActionHtml = ''
      if (this.filterValue === 'deleted') {
        groupActionHtml = '<button class="secondary">Restore</button>'
      } else if (this.viewValue === 'all') {
        const assignedGroups = clip.groups || []
        if (assignedGroups.length > 0) {
          const groupName = this.escapeHtml(assignedGroups[0])
          groupActionHtml = '<div class="group-badge">' + groupName + '</div>'
        } else {
          const options = this.groups.map(g => '<option value="' + this.escapeHtml(g.name) + '">' + this.escapeHtml(g.name) + '</option>').join('')
          groupActionHtml = '<select><option value="">Add to group...</option>' + options + '<option value="__new__">+ New Group...</option></select>'
        }
      } else {
        groupActionHtml = '<button class="secondary">Remove</button>'
      }
      ...
      div.innerHTML = dragHandle +
        '<div class="info">' +
        '<div class="name">' + this.escapeHtml(displayTitle) + '</div>' +
        '<div class="note' + noteClass + '">' + this.escapeHtml(noteText) + '</div>' +
        '</div>' +
        '<div class="actions">' +
        '<div class="rating-row">' +
        '<div class="star-rating-display">' + this.renderStarsDisplay(rating) + '</div>' + resultBadge +
        '</div>' +
        groupActionHtml +
        '</div>'
      ...
      div.addEventListener('click', (e) => {
        if (e.target.tagName === 'SELECT' || e.target.tagName === 'BUTTON' || e.target.classList.contains('drag-handle')) return
